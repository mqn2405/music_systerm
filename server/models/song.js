...
module.exports = {
  ...
  getUserListenData: async (userId) => {
    try {
      const list =
        await postgresql.query(`select s.category_id, s1."name", s1._id as singer_id, ult."time" from user_listen_time ult join songs s on s._id = ult.song_id
        join song_singer ss on ss.song_id = s._id
        join singers s1 on s1._id = ss.singer_id
        where ult.user_id = ${Number(userId)}
        order by ult."time" desc limit 2 offset 0
      `);
      const listRows = list?.rows;
      const fullList = await postgresql.query(`select s.*, c.name as country_name, al.name as album_name, ct.name as category_name from songs s 
        join countries c ON s.country_id = c._id 
        LEFT JOIN albums al ON s.album_id = al._id  
        JOIN categorys ct ON s.category_id = ct._id
        join song_singer ss ON ss.song_id = s._id
        join singers s1 ON s1._id = ss.singer_id
        where s.category_id = ${Number(listRows[0]?.category_id)} or s.category_id = ${Number(listRows[1]?.category_id)} or s1._id = ${Number(listRows[0]?.singer_id)} or s1._id = ${Number(listRows[1]?.singer_id)}`)
       return fullList;
    } catch (error) {
